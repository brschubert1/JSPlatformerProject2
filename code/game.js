...
var wobbleSpeed2 = 20, wobbleDist2 = 0.1;
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
  ...
  else if (type == "laser" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  }
   else if (type == "coin") {
    ...
    //make boost do something
    //fixxxxxxxxxxxxxxxxxxxxxxxx
    if (!this.actors.some(function(actor) {
           return actor.type == "boost";
         })) { //playerXSpeed = 14;
    function tempBoost() {
    setTimeout(boostStop, 3000);
    }
